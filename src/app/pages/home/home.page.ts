...
export class HomePage {
  ...
  stopRecord() {
    ...
    this.recording_datetime = new Date().getFullYear() + '-' + new Date().getMonth() + '-' + new Date().getDate() + '-' + new Date().getHours() + '-' + new Date().getMinutes() + '-' + new Date().getSeconds();
    ...
    formData.append('recording_datetime', this.recording_datetime);
    ...
    this.file.readAsArrayBuffer(this.filePath, this.fileName).then((data) => {
      ...
      console.log(file);
      this.apis.updateRecordingData(formData).subscribe(resData => {
        console.log(resData);
